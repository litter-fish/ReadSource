...
public abstract class AopConfigUtils {
	...
	private static BeanDefinition registerOrEscalateApcAsRequired(Class cls, BeanDefinitionRegistry registry, Object source) {
		...
		// 如果存在自动代理创建器且存在的自动代理器与现在的不一致，则需要根据优先级来判断使用哪个
		// AUTO_PROXY_CREATOR_BEAN_NAME = org.springframework.aop.config.internalAutoProxyCreator
		if (registry.containsBeanDefinition(AUTO_PROXY_CREATOR_BEAN_NAME)) { // 如果已经存在自动代理创建器
			BeanDefinition apcDefinition = registry.getBeanDefinition(AUTO_PROXY_CREATOR_BEAN_NAME);
			if (!cls.getName().equals(apcDefinition.getBeanClassName())) { // 自动代理器与现在的不一致
				int currentPriority = findPriorityForClass(apcDefinition.getBeanClassName());
				int requiredPriority = findPriorityForClass(cls);
				if (currentPriority < requiredPriority) { // 根据优先级判断使用哪个
					apcDefinition.setBeanClassName(cls.getName());
				}
			}
			// 如果存在的与现在的一致不需要创建直接返回
			return null;
		}
		...
		// 注册 beanName为 org.springframework.aop.config.internalAutoProxyCreator的BeanDefinition
